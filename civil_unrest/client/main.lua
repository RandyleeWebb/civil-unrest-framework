...
RegisterKeyMapping("+openCivilMenu", "Civil Unrest: System Interaction Radial", "pad", "INPUT_CONTEXT")
...
RegisterKeyMapping("+interactNPC", "Civil Unrest: Interrogate/Interact with AI", "pad", "INPUT_CONTEXT")
...
RegisterNUICallback("recruitCompanionWithName", function(data, cb)
    ...
    if currentTargetEntity then
        ...
        SetPedAsGroupMember(currentTargetEntity, group)
        SetPedNeverLeavesGroup(currentTargetEntity, true)
        GiveWeaponToPed(currentTargetEntity, GetHashKey("WEAPON_APPISTOL"), 500, false, true)
        ...
        SetPedPropIndex(currentTargetEntity, 0, 14, 0, false) -- Hat/Helmet identifier
        SetPedComponentVariation(currentTargetEntity, 8, 15, 0, 0) -- Undershirt/Accessory identifier
        
        -- Bond stats mapping (scaled by loyalty)
        local healthBonus = math.floor((loyalty / 100) * 200)
        SetEntityMaxHealth(currentTargetEntity, 400 + healthBonus)
        SetEntityHealth(currentTargetEntity, 400 + healthBonus)
        SetPedCombatAbility(currentTargetEntity, 100) 
        
        -- Advanced Combat Init natively
        SetPedCombatMovement(currentTargetEntity, 2) -- Offensive/Flanking
        SetPedCombatRange(currentTargetEntity, 1) -- Medium Range
        SetPedCombatAttributes(currentTargetEntity, 0, true) -- Can Use Cover
        SetPedCombatAttributes(currentTargetEntity, 5, true) -- Fight unarmed peds
        SetPedCombatAttributes(currentTargetEntity, 46, true) -- Fight armed peds
        
        ActiveCompanionPed = currentTargetEntity
...
AddEventHandler("civilUnrest:equipMerch", function(factionName, item)
    ...
    local playerPed = PlayerPedId()
    SetPedPropIndex(playerPed, 0, math.random(1, 10), 0, false)
...
AddEventHandler("civilUnrest:spawnNewsEvent", function(coords, eventType)
    Citizen.CreateThread(function()
        ...
        -- Command pathfinding
        TaskGoToCoordAnyMeans(reporterPed, coords.x, coords.y, coords.z, 1.5, 0, false, 786603, 0xbf800000)
